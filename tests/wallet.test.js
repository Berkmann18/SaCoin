...
test('Init', () => {
  ...
  expect(() => wlt.secretKey(String(hash ^ 0))).toThrowError(
    `A secret key recovery was attempted on the address ${wlt.address} with 3 attempts`
  );
  expect(() => wlt.secretKey(String(hash ^ 1))).toThrowError(
    'Secret key recovery attempt threshold exceeded.'
  );
  ...
  expect(wlt.toString()).toBe(
    `Wallet(blockchain=${wlt.blockchain.toString()}, address=${wlt.address}, publicKey=${
      wlt.publicKey.pubKeyHex
    })`
  );
  expect(wlt.toString(false)).toBe(
    `Wallet(blockchain=${wlt.blockchain.toString(false)}, address=${wlt.address}, publicKey=${
      wlt.publicKey.pubKeyHex
    })`
  );
  wlt.reset(hash);
